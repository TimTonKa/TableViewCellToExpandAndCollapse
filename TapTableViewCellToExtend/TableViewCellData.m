...
- (void) setAccessoryViewOnCell:(UITableViewCell*) cell {
    ...
    if (self.open) {
        ...
        if (self.titleAlternativeTextColor == nil) {
            ...
            cell.textLabel.textColor = self.titleAlternativeTextColor;
        }
    } else {
        path = @"DownAccessory";
    ...
    NSLog(@"%@",[[path stringByDeletingPathExtension] stringByAppendingString:@"White"]);
...
- (void) didSelectCellAtRow:(NSUInteger)row {
    if (row == 0){
        ...
        [self didSelectContentCellAtRow:row - 1];
    }
}
...
{
    NSLog(@"Content cell is selected!");
}
